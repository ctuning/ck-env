...
if [ "${PACKAGE_BUILD_TYPE}" == "configure" ] ; then
  ...
  if [ "${PACKAGE_SKIP_CONFIGURE}" != "YES" ] ; then
     cmake -DCMAKE_INSTALL_PREFIX="${INSTALL_DIR}/install" \
           ...
           -DCMAKE_CXX_FLAGS="${CK_CXX_FLAGS_FOR_CMAKE} ${CK_CXX_FLAGS_ANDROID_TYPICAL} ${PACKAGE_FLAGS} ${PACKAGE_FLAGS_LINUX} ${PACKAGE_FLAGS_ANDROID} ${CK_EXTRA_MISC_CXX_FLAGS}" \
           ${XCMAKE_AR} \
           ${XCMAKE_RANLIB} \
           ${XCMAKE_LD} \
           -DCMAKE_EXE_LINKER_FLAGS="${CK_LINKER_FLAGS_ANDROID_TYPICAL} ${CK_EXTRA_MISC_LINKER_FLAGS}" \
