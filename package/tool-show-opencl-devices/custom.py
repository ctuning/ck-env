...
def setup(i):
    ...
    # Check if Android 
    ana=tosd.get('android_ndk_arch','')
    texe1='show-opencl-devices'
    p=os.path.join(p, 'bin', 'android-'+ana,'ck-'+texe1+'.out')

    if ana!='' and os.path.isfile(p):
        if o=='con':
            ck.out('Reusing existing tool '+p)
    else:  
